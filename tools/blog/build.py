...
# Посилання всередині готового HTML із конвеєра мережі. Усе інше на сторінці
# складає href(), а ці адреси приходять уже написаними — і написаними
# по-українськи. Без цього читач російської статті з першого ж посилання в
# тексті потрапляв на українську сторінку: решта сайту при цьому лишалася
# російською, тож виглядало це як поламана навігація, а не як вибір мови.
_LINK = re.compile(r'href="((?:https://fluent-fox\.site)?/[^"]*)"')
...
def ru_links(html):
    def fix(m):
        ...
        path += ("&" if "?" in path else "?") + "lang=ru"
        return 'href="%s"' % (path + ("#" + frag if frag else ""))
    return _LINK.sub(fix, html)
...
def render_blocks(blocks, lang):
    ...
    for b in blocks:
        ...
        if kind == "p":
            ...
            # частин, і єдиний, який пише не людина. Єдине, що доводиться
            # чіпати, — адреси в тексті: решту посилань на сторінці збирає
            # href(), а ці приходять готовими й українськими.
            out.append(ru_links(b[1 + i]) if lang == "ru" else b[1 + i])
