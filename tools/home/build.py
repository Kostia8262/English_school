...
def squeeze(html):
    """Знімає відступи з готової розмітки.

    Навіщо. Шаблон вкладений глибоко, а `x-for` у секції «Програма»
    розгортається в 450 плашок тем — по три `<span>` на кожну, і кожна
    успадковує відступ у тридцять із лишком пробілів. У підсумку 208 тис.
    знаків файлу, тобто третина головної, — це самі лише відступи: більше,
    ніж увесь видимий текст сторінки разом із розміткою меню й підвалу.

    Кому це муляє. Не швидкості — сервер віддає сторінку стиснутою, і в
    gzip відступи майже нічого не важать. Муляє тим, хто читає сирий HTML:
    GPTBot, ClaudeBot і PerplexityBot обрізають документ за обсягом, а не
    за важливістю, і головна — єдина сторінка сайту, яка в типову межу не
    вкладалася. Обрізана вона саме там, де в неї лежить суть.

    Чому саме так, а не «справжня» мініфікація. Прибирається тільки те, що
    стоїть на початку рядка: сам перевід рядка лишається. Між двома
    рядковими елементами пропуск усе одно є, тож зібрана сторінка виглядає
    точно так само — жодного `display: inline` зі злиплими словами. Тому
    тут немає ні бібліотеки, ні списку винятків: `<pre>` і `<textarea>` на
    сайті не трапляються, а якби трапились, зламати їх цим правилом можна
    було б лише з відступом усередині.
    """
    html = re.sub(r"\n[ \t]+", "\n", html)
    return re.sub(r"\n{2,}", "\n", html)
